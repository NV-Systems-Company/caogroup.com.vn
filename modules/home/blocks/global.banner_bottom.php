...
if (!nv_function_exists('tms_banner_bottom')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function tms_config_banner_bottom($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function tms_config_banner_bottom_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function tms_banner_bottom($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
            ...
            $sql = 'SELECT title, title_extra, title_link, description, link, image, image_mobile, addtime, weight, status FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider_rows where bid='.$block_config['blockid'] . ' limit ' . $block_config['number_item'];
            $list = $nv_Cache->db($sql, '', $mod_name);

            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.banner_bottom.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.banner_bottom.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.banner_bottom.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
...
if (defined('NV_SYSTEM')) {
    $content = tms_banner_bottom($block_config);
}
