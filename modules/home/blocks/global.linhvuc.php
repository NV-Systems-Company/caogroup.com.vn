...
if (!nv_function_exists('nvb_home_danhmuc')) {
    ...
    function nvb_home_danhmuc($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
            ...
            $sql = 'SELECT title, title_extra, title_link, description, link, image, image_mobile, addtime, weight, status FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider_rows where bid='.$block_config['blockid'] . ' ORDER BY weight ASC' ;
            $list = $nv_Cache->db($sql, '', $mod_name);

            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.linhvuc.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.linhvuc.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.linhvuc.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
