...
if (!nv_function_exists('tms_congcu')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function tms_config_congcu($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function tms_config_congcu_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function tms_congcu($block_config)
    {
...
list ($titlebid, $descriptionbid) = $db_slave->query('SELECT title, description FROM ' . NV_PREFIXLANG . '_' . $mod_data . '_slider where bid='.$block_config['blockid'])->fetch(3);
            ...
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.lydo.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.lydo.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.lydo.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
...
if (defined('NV_SYSTEM')) {
    $content = tms_congcu($block_config);
}
