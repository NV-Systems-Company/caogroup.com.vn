...
if (!nv_function_exists('tms_intro_block')) {
    /**
     * @param string $module
     ...
     * @return string
     */
    function tms_intro($module, $data_block, $lang_block)
    {
     ...
     * @return number
     */
    function tms_intro_submit($module, $lang_block)
    {
        global $nv_Request;
     ...
     * @return string
     */
    function tms_intro_block($block_config)
    {
        ...
        if (isset($site_mods[$mod_name])) {
            ...
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/global.gioithieu.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $mod_file . '/global.gioithieu.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('global.gioithieu.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
            ...
            foreach ($list as $row) {
                ...
                $xtpl->assign('ROW', $row);
             if($i==1){$xtpl->parse('main.loop1');}
			 elseif($i==2){$xtpl->parse('main.loop2');}
			 elseif($i==3){$xtpl->parse('main.loop3');}
			 elseif($i==4){$xtpl->parse('main.loop4');}	 
...
if (defined('NV_SYSTEM')) {
    $content = tms_intro_block($block_config);
}
