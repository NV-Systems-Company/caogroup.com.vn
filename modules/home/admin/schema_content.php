...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $content = 'NO_' . $id;
    $query = 'SELECT status FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    $content = 'NO_' . $id;
    if ($new_vid > 0)     {
        $sql = 'SELECT bid,id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows WHERE  bid='.$bid.' AND id!=' . $id . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch())
        {
			...
			$sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows SET weight=' . $weight . ' WHERE bid='.$bid.' AND id=' . $row['id'];
            $db->query($sql);
        }
        $sql = 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows SET weight=' . $new_vid . ' WHERE bid='.$bid.' AND id=' . $id;
        $db->query($sql);
        $content = 'OK_' . $id;
    }
    $nv_Cache->delMod($module_name);
...
if ($nv_Request->isset_request('delete_id', 'get') and  $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        $weight=0;
        $sql = 'SELECT weight FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows WHERE id =' . $db->quote($id);
        $result = $db->query($sql);
        list($weight) = $result->fetch(3);
        
        $db->query('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows  WHERE id = ' . $db->quote($id));
        if ($weight > 0)         {
            $sql = 'SELECT id, weight FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows WHERE bid='.$bid.' AND weight >' . $weight;
            $result = $db->query($sql);
            while (list($id, $weight) = $result->fetch(3))
            {
                $weight--;
                $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows SET weight=' . $weight . ' WHERE bid='.$bid.' AND id=' . intval($id));
            }
        }
        $nv_Cache->delMod($module_name);
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete schema_content', 'ID: ' . $id, $admin_info['userid']);
...
$currentpath = NV_UPLOADS_DIR . '/' . $module_upload . '/schema';
if (!file_exists($currentpath)) {
    nv_mkdir(NV_UPLOADS_REAL_DIR . '/' . $module_upload, 'schema', true);
}
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                $row['addtime'] = 0;
				$weight = $db->query('SELECT max(weight) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows WHERE bid=' . $row['bid'])->fetchColumn();
                $weight = intval($weight) + 1;
                $stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows (bid, title, title_extra, title_link, description, link, image, image_mobile, addtime, weight, status) VALUES ('.$row['bid'].', :title, :title_extra, :title_link, :description, :link, :image, :image_mobile, '.NV_CURRENTTIME.', '.$weight.', :status)');
                $stmt->bindValue(':status', 1, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows SET title = :title, title_extra = :title_extra, title_link = :title_link, description = :description, link = :link, image = :image, image_mobile = :image_mobile WHERE id=' . $row['id']);
            }
            ...
            if ($exc) {
                $nv_Cache->delMod($module_name);
                if (empty($row['id'])) {
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Add schema_content', ' ', $admin_info['userid']);
                } else {
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit schema_content', 'ID: ' . $row['id'], $admin_info['userid']);
                }
    ...
    $row = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows WHERE id=' . $row['id'])->fetch();
    if (empty($row)) {
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()
        ->select('COUNT(*)')
		 ->where('bid ='.$row['bid'])
        ->from('' . NV_PREFIXLANG . '_' . $module_data . '_schema_rows');
    if (!empty($q)) {
...
$xtpl = new XTemplate('schema_content.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
$page_title = $lang_module['schema_content'];
