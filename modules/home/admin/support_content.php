    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_support_rows (bid, title, phone, email, skyper, zalo, facebook, image, addtime, weight, status) VALUES ('.$row['bid'].', :title, :phone, :email, :skyper, :zalo, :facebook, :image, '.NV_CURRENTTIME.', '.$weight.', :status)');
                $stmt->bindValue(':status', 1, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_support_rows SET title = :title,phone = :phone, email= :email, skyper = :skyper, zalo = :zalo, facebook= :facebook,  image = :image,  WHERE id=' . $row['id']);
            }
