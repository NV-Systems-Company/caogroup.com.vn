...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            if (empty($row['bid'])) {
                $stmt = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_slider (title, description, weight, status) VALUES (:title, :description, :weight, :status)');
                ...
                $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_slider SET title = :title, description = :description WHERE bid=' . $row['bid']);
            }
